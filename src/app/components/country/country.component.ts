...
import {BehaviorSubject, combineLatest, filter, map, Observable, startWith, take, tap} from "rxjs";
...
export class CountryComponent implements OnInit {
  protected yearOptions = [2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027, 2028, 2029, 2030];
  ...
  protected selectedCountryCode$ = this.selectedCountryCodeSubject.asObservable().pipe(filter(code => !!code));
  ...
  ngOnInit() {
    combineLatest([
      ...
      const isCountryCodeCorrect = countries.some(country => country.countryCode === params['code']);
      if (isCountryCodeCorrect) {
        this.selectedCountryCodeSubject.next(params['code']);
        this.store.dispatch(new FetchPublicHolidays(this.yearControl.value, params['code']));
      } else {
    ...
    this.selectedCountryInfo$ = combineLatest([
      ...
      map(([countriesInfoDict, selectedCountryCode]) => countriesInfoDict?.[selectedCountryCode])
    )
    ...
    this.selectedHolidays$ = combineLatest([
      ...
      map(([holidaysDict,  selectedCountryCode, selectedYear]) => holidaysDict?.[selectedCountryCode]?.[selectedYear]),
    );
    ...
    combineLatest([
      ...
      this.store.dispatch(new FetchPublicHolidays(selectedYear, selectedCountryCode));
    })
